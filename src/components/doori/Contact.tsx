      ...
      <div className="max-w-4xl mx-auto px-6 text-center">
        ...
        <div className="grid sm:grid-cols-3 gap-5 mb-12">
          {CHANNELS.map(({ icon, label, value, sub }) => (
            <div
              key={label}
              className="bg-slate-50 border border-slate-100 p-7 flex flex-col items-center gap-3
                card-hover hover:border-blue-100 hover:shadow-lg hover:shadow-blue-50"
            >
              <div className="w-11 h-11 bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center text-white shadow-sm">
                {icon}
              </div>
              <span className="text-gray-400 text-xs tracking-wide">{label}</span>
              <span className="text-gray-800 font-bold text-sm text-center leading-snug">{value}</span>
